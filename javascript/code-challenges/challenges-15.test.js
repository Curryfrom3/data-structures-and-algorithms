...
let biggerThanLuke = (arr) => {
  ...
  const charactersWithGreaterMass = arr.filter(character => {
    ...
    return !isNaN(characterMass) && characterMass > lukesMass;
  });

  const namesWithGreaterMass = charactersWithGreaterMass.map(character => character.name);
  const joinedNames = namesWithGreaterMass.join(' - ');
  return joinedNames;
