...
const toLastNames = people => {
  let fullNames = people.map((element) => `${element.firstName} ${element.lastName}`);
  return fullNames;
};
...
const addValues = (arr) => {
  let initalValue = 0;
  let sum = arr.reduce((accumulator, currentValue) => accumulator + currentValue, initalValue);
  return sum;
};
...
const addPurchases = (arr) => {
  let itemPrices = arr.map((element) => element.purchasePrice);
  let initalValue = 0;
  let totalPrice = itemPrices.reduce((accumulator, currentValue) => accumulator + currentValue, initalValue);
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  let result = arr.reduce(function (a) { return a + 1; }, 0);
  return result;
};
...
const returnNames = (arr) => {
  let nameCollection = arr.reduce((namesArr, character) => {
    let charName = character.name;
    namesArr.push(charName);
    return namesArr;
...
const reversedString = (str) => {
  return str.split('').reduce((stringHolder, letter) => letter + stringHolder, '');
}
