...
const count = (target, input) => {
  return input.reduce((count, subArr) => {
    return count + subArr.filter((n) => n === target).length;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(subArr => {
    return subArr.filter(num => typeof num === 'number' && num % 5 === 0)
...
let findShortest = (data) => {
  const shortestPerson = data
    .filter(person => person.height !== 'n/a') // Filter out people with unknown height
    .map(person => {
      return {
        name: person.name,
    ...
    .reduce((shortest, current) => {
      if (current.height < shortest.height) {
        return current;
      } else {
        return shortest;
      }
  ...
  return shortestPerson.name; // Return the name of the person with the shortest height
